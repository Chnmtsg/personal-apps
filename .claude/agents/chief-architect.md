...
Read the reviewed application's own `project.md` — `arise/knowledge/project.md` for
arise, `knowledge/project.md` for english-feedback-app. Your decisions must serve
the vision, the hard constraints and the long-term roadmap in that file.
...
Durability and correctness of the user's stored writing outranks everything else. It exists only on their device.

Never approve anything that puts the Anthropic API key, or any secret, in client code.
